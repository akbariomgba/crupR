...
getEnhancers <-
    function(data, classifier = "default", cutoff = 0.5, distance = 12500,
    cores = 1) {
    ##################################################################
    ## start run time
    ##################################################################
    ...
    for (feature in features_all) {
        ...
        GenomicRanges::mcols(data_matrix_norm)[, feature] <- preprocessCore::normalize.quantiles.use.target(matrix(GenomicRanges::mcols(data_matrix_norm)[,
    ...
    # normalized matrix
    data_matrix_norm_ext <- extend_dataMatrix(N = 5, df = data.frame(data_matrix_norm),
        f = features_all)
    ...
    prediction1 <- predict(classifier1, data_matrix_norm_ext[seq_len(mid), ], type = "prob")[,
        2]
    prediction1 <- c(prediction1, predict(classifier1, data_matrix_norm_ext[(mid +
        1):nrow(data_matrix_norm_ext), ], type = "prob")[, 2])

    prediction2 <- predict(classifier2, data_matrix_norm_ext[seq_len(mid), ], type = "prob")[,
        2]
    prediction2 <- c(prediction2, predict(classifier2, data_matrix_norm_ext[(mid +
        1):nrow(data_matrix_norm_ext), ], type = "prob")[, 2])
