...
#' @param all Output of separated probabilities. Default=FALSE
...
#' norm <- list(metaData = metaData, normalized = d)
...
getEnhancers <- function(data, classifier = NULL, cutoff = 0.5, distance = 12500, C = 1, all = FALSE){
  start_time <- Sys.time()
  cat('\n')
  
  if (!is.null(classifier) && (!dir.exists(classifier))) stop(paste0(classifier, " is not a valid directory"))
  if (!is.null(cutoff) & (cutoff < 0 | cutoff > 1) ) stop(paste0(cutoff, " is not in range [0,1]."))
