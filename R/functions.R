...
get_binned_genome <- function(txdb, width){
  ...
  binned <- GenomicRanges::tileGenome(GenomeInfoDb::seqinfo(txdb),
                       tilewidth = width,
                       cut.last.tile.in.chrom = TRUE)

  # only take autosomes and X chromosome
  GenomeInfoDb::seqlevels(binned, pruning.mode = "coarse") <- GenomeInfoDb::seqlevels(txdb)[grep("^chr[0-9]{,2}$|chrX$", GenomeInfoDb::seqlevels(txdb))]
...
get_pairwisePvalues <- function(p, I, w_0, W, p.thres, C){
  ...
  for(i in seq(ncol(comb))){
    ...
    D <- unlist(parallel::mclapply( idx,
                                     FUN = function(x)  get_KS.STATISTIC( z[(x - W):(x + W),], W),
                                     mc.cores = C,
    ...
    if(length(idx != 0)) {
        ...
        GenomicRanges::mcols(this.result) <- data.frame(p.value = p.value[idx.significant],
                                                        ...
                                                        p.direction = unlist(lapply(idx.significant, function(x) (mean(z[(x - W):(x + W),1]) - mean(z[(x - W):(x + W),2]) <= 0))), #p.direction = (c1m - c2m)[idx.significant] <= 0,
                                                        idx = idx.significant,
...
get_ranges <- function(p, I, W, C){
  ...
  GenomicRanges::mcols(p.red)[, unlist(I)] <- do.call("rbind", parallel::mclapply(p.split2,
                                                                                  ...
                                                                                  FUN = function(x) matrixStats::colMedians(as.matrix(GenomicRanges::mcols(x)[,unlist(I)])),
...
name_patterns <- function(gr, C){
  ...
  sizes <- vapply(seq_along(patterns), function(i) length(which(gr$pattern == patterns[i])), FUN.VALUE = numeric(1))
...
plotSummary <- function(dynamic_enhancers, num_plots = 20){
  ...
  ggplot2::ggplot(d, ggplot2::aes(x=condition, y=value, col=condition, shape=replicate)) +
    ...
    ggplot2::xlab('') + ggplot2::ylab('max. enh. prob. in range')
}
...
get_units <- function(regions.gr, expr.gr, TAD.gr, IDs, cores, threshold, txdb, nearest = FALSE){
