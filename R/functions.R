...
get_binned_genome <- function(genome, chr=NULL){
  ...
  } else {GenomeInfoDb::seqlevels(genome) <- GenomeInfoDb::seqlevels(txdb)[grep("^chr[0-9]{,2}$|chrX$", GenomeInfoDb::seqlevels(txdb))]}
...
get_cluster <- function(p, pvalues, I){
  ...
  p$sign[idx] <- TRUE
  p$pattern <- 0
  p$pattern[idx] <- apply(m[,-max(comb.f)], 1, function(x) x%*%(2^(seq(length(x))-1))) 
  p$pattern[intersect(which(p$pattern == 0), idx[which(m[,max(comb.f)] == 1)])] <- 'ubiquitous'
  return(p)
}
...
get_ranges <- function(p, I, W, C){
 	p.split <- split(p, seqnames(p))
	rle <- lapply(p.split, function(x) rle(x$pattern))
	...
	d$pattern <- unlist(lapply(rle, function(x) x$values))
	d$start <- unlist(lapply(p.split, function(x) start(x)[d$start.idx[which(d$seqnames ==unique(seqnames(x)))]]))
	d$end <- unlist(lapply(p.split, function(x) end(x)[d$end.idx[which(d$seqnames ==unique(seqnames(x)))]]))
	gr <- makeGRangesFromDataFrame(d[,-grep('.idx', colnames(d))], keep.extra.columns=T)
	
	ovrlp <- findOverlaps(p, gr)[which(p$pattern !=0)]
	ovrlp.gr<-p[queryHits(ovrlp)]
	ovrlp.split <- S4Vectors::split(ovrlp.gr, subjectHits(ovrlp))
	names(ovrlp.split) <- gr$pattern[unique(subjectHits(ovrlp))]
	ovrlp.red <- reduce(ovrlp.split)
	...
	ret$pattern <- names(ovrlp.red)
	tab <- rev(table(ret$pattern))
	pattern.names <- paste0('c',seq(length(tab)))
	names(pattern.names) <- names(tab)
	ret$pattern <- pattern.names[ret$pattern]
	
	GenomicRanges::mcols(ret)[, unlist(I)] <- do.call("rbind",
                                                      parallel::mclapply(
                                                      split.final,
                                                      FUN = function(x) colMaxs(as.matrix(GenomicRanges::mcols(x)[,unlist(I)])),
                                                      mc.cores = C,
                                                      mc.allow.recursive = FALSE
                                                      )
  	)
...
plotSummary <- function(D, num_plots = 9){
  ...
  if(length(unique(p$pattern)) > num_plots)  p <- p[which(p$pattern %in% paste0("c",seq_len(num_plots)))]
  ...
  d$label <- paste0(p$pattern, ' (', tab[p$pattern], ' regions)') 
  ...
  if (sum(this.genes.idx) > 0) {
    ...
    for (c in seq_along(cor)) {
      ...
      interactions <- rbind(interactions,
                            data.frame(data.frame(this.region)[,c(GR_header_short, "pattern", I)],
...
get_nearest_gene <- function(i, regions.gr, genes, I){
  ...
  interactions <- data.frame(data.frame(this.region)[,c(GR_header_short, "pattern", I)],
