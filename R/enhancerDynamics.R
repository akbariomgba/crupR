...
#' inp <- rep(system.file('extdata', 'Condition1.Input.bam', package='crupR'),3)
#' inp2 <- rep(system.file('extdata','Condition2.Input.bam', package='crupR'),3)
#' metaData <- data.frame(HM = rep(c('H3K4me1', 'H3K4me3', 'H3K27ac'),2),
#'                 condition = c(1,1,1,2,2,2), replicate = c(1,1,1,1,1,1),
#'                 bamFile = files, inputFile = c(inp, inp2))
...
getDynamics <- function(data, w_0 = 0.5,
    ...
    metaData <- do.call("rbind", lapply(data,
        function(x) x$metaData))
    conds <- unique(metaData$condition)
    num_cond <- length(conds)
    IDs <- list()
    for (i in seq_len(num_cond)) {
        cond_subset <- subset(metaData,
            condition == conds[i])
        IDs[[i]] <- paste0(ID_prefix,
            conds[i], "_", unique(cond_subset$replicate))
    }
    comb <- (combn(seq(length(unique(metaData$condition))),
        2))
    comb.list <- lapply(seq_len(ncol(comb)),
        function(i) comb[, i])


    if (threshold < 0 | threshold >
        1) {
        message <- paste0(threshold,
            " is not in range [0, 1].")
        stop(message)
    }
    ...
    for (i in seq_len(length(data))) {
        ...
        probs <- GenomicRanges::merge(probs,
            matrix, all = TRUE)
    }
    ...
    ##################################################################
    ## get empricial p-values (list with empirical p-values
    ## per pair comparison and direction of group mean differences)
    ##################################################################

    startPart("Calculate (pairwise) empirical p-values")
    p <- get_pairwisePvalues(p = probs,
        I = IDs, w_0 = w_0, W = window,
    ...
    probs <- get_cluster(p = probs,
        pvalues = p, I = IDs)
    done()

    if ((length(unique(probs$pattern)) ==
        1) && (unique(probs$pattern) ==
        paste(rep("0", num_cond),
            collapse = ""))) {
        message <- paste0(skip(),
    ...
    peaks <- get_ranges(p = probs,
        I = IDs, W = window, C = cores)
    rm(probs)
    done()

    cat(paste0(skip(), "assign clusters"))
    peaks <- name_patterns(gr = peaks,
        C = cores)
    done()
