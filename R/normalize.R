...
normalize <- function(metaData,condition,replicate, genome, mapq = 10, sequencing, input.free = FALSE, chroms = "all", cores = 1) {
  ##################################################################
  # start run time
  ##################################################################
  ...
  # check summary file
  m <- metaData[which(metaData$condition == condition & metaData$replicate == replicate),] #get relevant rows
  ...
  bam_header <- Rsamtools::scanBamHeader(bam_files_HM[1])[[1]]$targets
  done()
  ...
  if(input.free == TRUE) names <- features.valid
  ...
  if (sequencing == "paired") {
      counts <- parallel::mcmapply(
                bamsignals::bamProfile, bampath = bams,
                MoreArgs = list(gr = binned_genome, binsize = 100,
                mapqual = mapq,
                ss = FALSE,
                paired.end = "midpoint",
                filteredFlag = 1024,
  ...
  GenomicRanges::mcols(binned_genome) <-  matrix( unlist(normalized_counts),
                                   ncol = 3,
                                   byrow = FALSE,
  ...
  nominator   <- GenomicRanges::mcols(binned_genome)[,"H3K4me1"] + abs(min(GenomicRanges::mcols(binned_genome)[,"H3K4me1"])) + 1
  denominator <- GenomicRanges::mcols(binned_genome)[,"H3K4me3"] + abs(min(GenomicRanges::mcols(binned_genome)[,"H3K4me3"])) + 1
  GenomicRanges::mcols(binned_genome)[,"ratio"] <- log2(nominator/denominator)
  done()
