...
#' genome = Seqinfo(seqnames=c("chr3", "chr4", "chrM"),
...
normalize <- function(metaData, condition, replicate, genome, mapq = 10, sequencing, input.free = FALSE, chroms = NULL, C = 1) {
  ...
  if (class(genome)!='Seqinfo' && genome == "mm10") {genome <- GenomeInfoDb::seqinfo(BSgenome.Mmusculus.UCSC.mm10::BSgenome.Mmusculus.UCSC.mm10)
  }  else if (class(genome)!='Seqinfo' && genome == "mm9") {genome  <- GenomeInfoDb::seqinfo(BSgenome.Mmusculus.UCSC.mm9::BSgenome.Mmusculus.UCSC.mm9)
  }  else if (class(genome)!='Seqinfo'&& genome == "hg19") {genome <- GenomeInfoDb::seqinfo(BSgenome.Hsapiens.UCSC.hg19::BSgenome.Hsapiens.UCSC.hg19)
  }  else if (class(genome)!='Seqinfo' && genome == "hg38") {genome <- GenomeInfoDb::seqinfo(BSgenome.Hsapiens.UCSC.hg38::BSgenome.Hsapiens.UCSC.hg38)
  }  else if(class(genome)!='Seqinfo') {stop(paste0("Your genome is neither one of ",paste0(genome_values, collapse=',')," nor is it a valid Seqinfo object."))}
