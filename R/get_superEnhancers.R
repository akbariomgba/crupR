...
#' in <- rep(system.file("extdata", "Condition2.Input.bam",package="crupR"), 3)
...
getSE <- function(data, cutoff = 0.5, distance = 12500, cores = 1){
  ...
  if(cutoff < 0 | cutoff>1){
    message = paste0(cutoff,
        " is not a valid cutoff. Please choose a cutoff between 0 and 1.")
    stop(message)
  }

  if(distance < 0){
    message = paste0(distance,
        " is not a valid distance. Please choose a distance greater than 0.")
    stop(message)
  }
  ...
  output <- list("metaData" = data$metaData,
                 "data_matrix" = data$data_matrix,
                 "peaks" = peaks, "cluster" = cluster)
  return(output)
}
