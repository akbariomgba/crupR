...
getTargets <- function(data, expression, genome, TAD.file = "default", threshold = 0.9,
    nearest = FALSE, cores = 1) {
    ...
    if (nearest == FALSE) {
        ...
        units <- get_units(regions.gr, expr.gr, TAD.gr, IDs, cores, threshold, txdb)
    } else {
        units <- get_units(regions.gr, expr.gr, TAD.gr, IDs, cores, threshold, txdb,
            nearest = TRUE)
    }
